...
import { collection, getDocs, query, where, doc, updateDoc, getDoc, addDoc, serverTimestamp, getCountFromServer, documentId, onSnapshot } from 'firebase/firestore';
...
export function SubmissionManagement({ showOnlyPending = false }: SubmissionManagementProps) {
      ...
      const unsubscribe = onSnapshot(commentsQuery, async (snapshot) => {
        ...
        try {
          ...
          setComments(prev => {
            ...
            console.log('Comment count for submission:', submissionId, ':', validComments.length);
            return newComments;
          });
        } catch (error) {
  ...
  useEffect(() => {
    if (selectedSubmissionForComments && comments[selectedSubmissionForComments] && comments[selectedSubmissionForComments].length > 0) {
      commentsEndRef.current?.scrollIntoView({ behavior: 'smooth' });
    }
  }, [selectedSubmissionForComments, comments]);
  ...
  }, [submissions, fetchSubmissionDirectly]);
                      ...
                      <div className="mt-4">
                        <button
                          onClick={() => {
                            ...
                            if (newSelected) {
                              console.log('🖱️ [SubmissionManagement] Opening comments for:', newSelected);
                              // Mark as viewed for badge display
                              setViewedSubmissions(prev => new Set([...prev, newSelected]));
                            }
                          }}
